...
if(isset($_SESSION['uname']) && isset($_SESSION['uid']) && isset($_SESSION['uemail']) && isset($_SESSION['uimg']))
{
    if(isset($_POST['submit']))
    {
        ...
        $q2="insert into complain values('$new','$stitle','$desc','$oid','$file1','$uid','$uname','$uemail','$mno','USER','0',current_timestamp())";
        if($r2=mysqli_query($conn,$q2))
        {
            $msg="Complain Added SuccesFully";
        }
        else
        {
            $error="Complain Not Added SuccesFully";
        }
                                    ...
                                    <form class="forms-sample" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="title" name="title"
                                                placeholder="Complain Title" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="gender">Order-ID</label>
                                            <select class="form-control form-control-lg" id="oid" name="oid"  >
                                                <option value="">Order-ID(Optional)</option>
                                                <?php
                                                $q5="select * from orders where uid='".$_SESSION['uid']."'";
                                                if($r5=mysqli_query($conn,$q5))
                                                {
                                                    while($num=mysqli_fetch_assoc($r5))
                                                    {
                                                        echo '<option value="'.$num['order_id'].'">'.$num['order_id'].'</option>';
                                                    }
                                                } 
                                                ?>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="file" name="complain" class="file-upload-default" accept="image/x-png,image/gif,image/jpeg,application/pdf,application/msword,
