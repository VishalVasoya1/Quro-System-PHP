...
if(isset($_SESSION['uname']) && isset($_SESSION['uid']) && isset($_SESSION['uemail']) && isset($_SESSION['uimg']))
{
  if(isset($_POST['submit']))
  {
      ...
      if(!$file1 == "")
      {
        unlink("../../images/user/profile/".$eimg);
        move_uploaded_file($temp,"../../images/user/profile/$file1");
        $q11="update user_register set uimg='$file1' where uid='$uid'";
      ...
      if(!$uemail == "")
      {
          ...
          {
                $q11="select * from user_register where uemail='$uemail'";
                if($r11=mysqli_query($conn,$q11))
                {
                    $n11=mysqli_num_rows($r11);
                    if($n11<=0)
                    {
                        $q12="update user_register set uemail='$uemail' where uid='$uid'";
       ...
       if(!$umno == "")
      {
          ...
          {
                $q13="select * from user_register where umno='$umno'";
                if($r13=mysqli_query($conn,$q13))
                {
                    $n12=mysqli_num_rows($r13);
                    if($n12<=0)
                    {
                        $q14="update user_register set umno='$umno' where uid='$uid'";
            ...
            $q1="update user_register set uname='$ename',gender='$egender',stamp=current_timestamp() where uid='$uid'";
            if($r1=mysqli_query($conn,$q1))
            {
                $msg="<b><u>   ". $ename ."</u></b>". " User Updated SuccesFully....";
                header("refresh:2,url=user_profile.php");
            }
            else
            {
                $error="<b><u>   ". $ename ."</u></b>". " User Updation Failed....";
            }
    }
}
?>
